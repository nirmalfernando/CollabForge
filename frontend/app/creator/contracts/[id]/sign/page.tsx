...
import { Input } from "@/components/ui/input" // Import Input for digital signature
import Header from "@/components/header"
import Footer from "@/components/footer"
import { toast } from "@/hooks/use-toast"
import { useRouter } from "next/navigation"

export default function CreatorContractSignPage({ params }: { params: { id: string } }) {
...
• Croque monsieur cut the cheese st. agur blue cheese squirty cheese pepper jack macaroni cheese.`,
  ...
  const handleAgreeAndSign = (e: React.FormEvent) => {
    e.preventDefault()
    if (!agreedToTerms) {
      toast({
        title: "Agreement Required",
        description: "Please agree to the terms and conditions to sign the contract.",
        variant: "destructive",
      })
      return
    }
    ...
    console.log("Contract Signed by:", digitalSignature)
    console.log("Creator Suggestions:", creatorSuggestions)
    toast({
      title: "Contract Signed",
      description: "You have successfully signed the contract!",
    })
    // In a real app, you would send this data to your backend
    router.push("/creator/applications") // Redirect back to applications page
  }
        ...
        <div className="container mx-auto max-w-4xl space-y-8">
          ...
          <div className="space-y-4">
            ...
            <Textarea
              ...
              value={creatorSuggestions}
              onChange={(e) => setCreatorSuggestions(e.target.value)}
              rows={8}
