...
const CreatorChatsPage: React.FC = () => {
  // State management
  ...
  // Handle URL parameters for direct contact (if someone wants to message the creator)
  useEffect(() => {
    const contactUserId = searchParams?.get('contactUserId');
    const contactName = searchParams?.get('contactName');
    const autoMessage = searchParams?.get('autoMessage') === 'true';
    
    console.log('Creator chat URL parameters:', { contactUserId, contactName, autoMessage });
  ...
  const selectConversation = useCallback((conversation: Conversation) => {
    ...
    // Join new conversation room if it exists
  ...
  const sendInitialContactMessage = async (conversation: Conversation) => {
    try {
      const initialMessage = `Hello! Thank you for your interest in collaborating. I'd be happy to discuss potential opportunities with you.`;
  ...
  if (loading) {
    return (
      <div className="flex flex-col min-h-screen bg-black text-white">
        <Header isLoggedIn={true} userRole="creator" />
          ...
          <ul>
            {conversations.length === 0 ? (
              <div className="px-6 py-8 text-center text-gray-400">
                <p>No messages yet</p>
                <p className="text-sm mt-2">Brands will reach out to you for collaborations</p>
              </div>
            ) : (
              conversations.map((conversation) => (
                <li
                  key={conversation.conversationId || conversation.otherParticipant.userId}
                  className={`flex items-start px-6 py-3 cursor-pointer transition-colors ${
                    selectedConversation?.otherParticipant.userId === conversation.otherParticipant.userId
                      ? "bg-gray-800"
                      : "hover:bg-gray-800"
                  }`}
                  ...
                  <div className="ml-3 flex-1 min-w-0">
                    ...
                    <div className="flex justify-between items-center">
                      <p className="text-sm text-gray-300 truncate">
                        {conversation.lastMessage?.message || "New collaboration opportunity"}
                      </p>
          ...
          {selectedConversation ? (
                ...
                {!selectedConversation.conversationId && messages.length === 0 && (
                  <div className="flex justify-center">
                    <div className="bg-gray-800 rounded-lg px-4 py-2 text-center text-gray-300">
                      <p className="text-sm">
                        New collaboration opportunity with {selectedConversation.otherParticipant.name}
                      </p>
            ...
            <div className="flex flex-col items-center justify-center flex-1 text-gray-400">
              <div className="text-center">
                ...
                <p>Choose a message from brands to start discussing collaborations</p>
                {!isConnected && (
