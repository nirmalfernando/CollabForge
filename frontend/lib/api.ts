...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_BASE_URL || "https://collabforge.onrender.com/api"
...
export const creatorApi = {
  createCreator: async (creatorData: {
    ...
    socialMedia?: Array<{ platform: string; handle: string; url: string; followers?: number }>
    ...
    pastCollaborations?: Array<{ brand: string; campaign: string; date?: string }>
    categoryId: string
  ...
  uploadImage: async (file: File): Promise<{ url: string }> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ApiError(response.status, data.error?.message || "Failed to upload image to Cloudinary", data)
      }
      ...
      console.error("Network or unexpected error during Cloudinary upload:", error)
